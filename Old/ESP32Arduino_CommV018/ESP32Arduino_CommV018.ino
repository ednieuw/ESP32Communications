...
static bool                    TSClientConnected   = false;                                     // Comment line ...
static int                     TSlastMinute        = -1;
...
void setup() 
{
 ...
 if(Mem.TimeReceiver) {LEDstartup(blue); StartTimeSenderScan();Tekstprintln("Time Receiver started"); } // Time Sender app on iPhone can be used
...
void CheckDevices(void)
{
 ...
 if (Mem.TimeReceiver) CheckTimeSenderClient();  
 if (Mem.TimeSender)   CheckTimeSender();                                                                                                     // Check serial port every second 
...
void EverySecondCheck(void)
{
 ...
 if (msLeap >999)                                                                             // Every second enter the loop
 {
  ...
  if (shouldReboot) { delay(3000);   ESP.restart(); }                                         // After finish OTA update restart
...
{   
 ...
 NoTextInLeds = ff;                                                                                // If Mem.TimeMinute == 0 the time (controlled in ColorLeds()
...
void UpdateStatusLEDs(bool Toggle)
{
 if(Mem.StatusLEDOn)   
   {
    if (Toggle) SetStatusLED(
                 (WiFi.localIP()[0]==0) * 20,                                                 // no WIFI on 
                 (WiFi.localIP()[0]!=0) * 20,                                                 // WIFI on
                 (BLEConnected)         * 20 );                                               // BLE on
    else SetStatusLED(TSConnected,0,TSConnected);                                             // if TSConnected then yellow Red+BLue   
...
void InitStorage(void)
{
 ...
 Mem.WIFInoConnection = 0;                                                                    // Reset the restart No connection counter
 StoreStructInFlashMemory();
}
//--------------------------------------------                                                //
...
void GetStructFromFlashMemory(void)
{
 ...
 FLASHSTOR.end(); 
 Tekstprintlnf("Mem.Checksum = %d",Mem.Checksum);
}
//--------------------------------------------                                                //
...
{ 
 ...
 PrintLine(35);
 snprintf(sptext, sizeof(sptext),"Display off between: %02dh - %02dh",
               Mem.TurnOffLEDsAtHH, Mem.TurnOnLEDsAtHH);                                        WTekstprintln(sptext);
...
void PrintMem(void)
{
 ...
 Tekstprintlnf("   Mem.UseRotary: %d",Mem.UseRotary);                       
...
//--------------------------------------------                                                //
// BLE TimeReceiver Connect TimeSender
// -------------------------------------------
...
//--------------------------------------------                                                //
// BLE TimeReceiver Check TimeSender Client
//--------------------------------------------
...
void WebPage(void) 
{
 ...
 server.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request)                              // Send a GET request to <ESP_IP>/get?input1=<inputMessage>
   { 
    ...
    if ( !inputMessage.equalsIgnoreCase("Y") )                                                // Function that runs >500 ms will crash. Executes with CheckWIFIcommand() in the loop() 
        { ReworkInputString(inputMessage);    PendingCommand = "";   }
...
void BLETimeSenderStart(void)
{
  ...
  TSpService = TSpServer->createService(UART_SERVICE_UUID);                                     // Comment line ...

  TSpTX = TSpService->createCharacteristic(
              UART_CHARACTERISTIC_TX,
              NIMBLE_PROPERTY::NOTIFY
           );                                            

  TSpRX = TSpService->createCharacteristic(
...
void CheckTimeSender(void)
{
 ...
 if(m != TSlastMinute)
   {
    char buffer[16];                                     
    sprintf(buffer,"T%02d%02d%02d",h,m,s);                
 ...
 if(TSdatePending && (millis() - TSsendDelayms > 1000))
   {
    char buffer[16];                                     
    sprintf(buffer,"D%02d%02d%04d",d,mo,y);               
    TSpTX->setValue((uint8_t*)buffer, strlen(buffer));   
    TSpTX->notify();                                     
    TSdatePending = false;                               
   }
