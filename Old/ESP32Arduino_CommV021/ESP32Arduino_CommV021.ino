...
void setup() 
{
 ...
 if(Mem.MCUrestarted>5) { Reset();  }                                                        // If the MCU restarts during Setup() so often Reset but not credentials 
 ...
 SWversion();                                                                                 // Print the menu + version                                                                        // Startup went well; Set MCUrestart counter to 0    
 StoreStructInFlashMemory();                                                                  // 
 delay(250);
...
{   
 ...
 DimLeds(!Mem.TimeLogPrint);                                                                    // and DimLEDs are used to print the time every minute
...
{ 
 ...
 if (!NoTextInLeds && !Mem.TimeLogPrint && strlen(Tekst) > 0 )
 ...
 server.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request)                              // Send a GET request to <ESP_IP>/get?input1=<inputMessage>
   { 
    ...
    if (inputMessage.equals("i")  || inputMessage.equals("ii") )                              // The SWversion() is printed to the web page to update the settings of the web page
         DoNotLog = true;                                                                     // but this should not be logged in the Logviewer only when I or II is entered.  
